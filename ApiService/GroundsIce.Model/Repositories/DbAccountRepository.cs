namespace GroundsIce.Model.Repositories
{
    ...
    public class DbAccountRepository : IAccountRepository
    {
        ...
        public async Task<Account> CreateAccountAsync(Login login, Password password)
        {
            if (login == null)
            {
            ...
            using (var connection = await this.connectionFactory.GetConnectionAsync())
            {
                try
                {
                    long userId = connection.Insert(new DbAccount { Login = login.Value, Password = password.Value });
        ...
        public async Task<Account> GetAccountAsync(Login login, Password password)
        {
            if (login == null)
            {
                throw new ArgumentNullException("login");
            ...
            return await this.GetAccountBySqlParamsAsync(sqlQuery, new { Login = login.Value, Password = password.Value });
        }
        ...
        public async Task<bool> ChangeLoginAsync(long userId, Login newLogin)
        {
            if (userId < 0)
            {
                throw new ArgumentOutOfRangeException("userId");
            ...
            using (var connection = await this.connectionFactory.GetConnectionAsync())
            {
                try
                {
                    ...
                    int changedRows = await connection.ExecuteAsync(sqlQuery, new { UserId = userId, NewLogin = newLogin.Value });
        ...
        public async Task<bool> ChangePasswordAsync(long userId, Password oldPassword, Password newPassword)
        {
            if (userId < 0)
            {
                throw new ArgumentOutOfRangeException("userId");
            ...
            using (var connection = await this.connectionFactory.GetConnectionAsync())
            {
                int changedRows = await connection.ExecuteAsync(
                    sqlQuery,
                    new { UserId = userId, OldPassword = oldPassword.Value, NewPassword = newPassword.Value });
        ...
        private async Task<Account> GetAccountBySqlParamsAsync<T>(string sql, T sqlParams)
        {
            using (var connection = await this.connectionFactory.GetConnectionAsync())
            {
                ...
                return dbAccount != null ? new Account(dbAccount.UserId, new Login(dbAccount.Login)) : null;
            }
        }
